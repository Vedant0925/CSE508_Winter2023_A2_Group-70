...
def tfidf(documents, vocab_matrix, tf_func):
    ...
    idf = {}
    for term in vocab_matrix:
        idf[term] = math.log(len(documents) / (doc_freq[term] ))

    # Calculate TF-IDF matrix for each document using IDF values
    tfidf_matrix = []
    for doc in documents:
        tfidf_vector = []
        for term in vocab_matrix:
            tf = tf_func(term, doc)
            tfidf_vector.append(tf * idf[term])
...
def construct_query_vector(query, vocab_matrix):
    ...
    query_vector = [1 if term in query_terms else 0 for term in vocab_matrix]
    return query_vector
...
def compute_tfidf_score(query_vector, tfidf_matrix):
    ...
    for i, tfidf_vector in enumerate(tfidf_matrix):
        ...
        scores[0].append((i, score))
    scores[0].sort(key=lambda x: x[1], reverse=True)
    return scores[0]
...
ranked_docs = sorted(jaccard_scores, key=sort_by_second_element, reverse=True)[:10]
